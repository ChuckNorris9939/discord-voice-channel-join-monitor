...
DB_KEY_TESTING_CHANNEL_ID = "TESTING_CHANNEL_ID"
DB_KEY_AFK_CHANNEL_ID = "AFK_CHANNEL_ID"
# Web UI access control. Values are Authentik group names, matched against the
# X-authentik-groups header the proxy sends. An empty value means no restriction.
DB_KEY_ACCESS_GROUP = "ACCESS_GROUP"
DB_KEY_JOIN_LOGS_GROUP = "JOIN_LOGS_GROUP"
DB_KEY_SETTINGS_GROUP = "SETTINGS_GROUP"
DB_KEY_BOT_CONTROL_GROUP = "BOT_CONTROL_GROUP"
DB_KEY_RECORDINGS_GROUP = "RECORDINGS_GROUP"
...
DB_KEY_GARMIN_SILENCE_COMPRESSION_ENABLED = "GARMIN_SILENCE_COMPRESSION_ENABLED"
DB_KEY_TEMP_DAVE_FIX = "TEMP_DAVE_FIX"
...
AFK_CHANNEL_ID = 0
# Empty by default: nothing is locked down until the groups are filled in via
# the settings page, so deploying this cannot lock anyone out.
ACCESS_GROUP = ""
JOIN_LOGS_GROUP = ""
SETTINGS_GROUP = ""
BOT_CONTROL_GROUP = ""
...
# Temp DAVE Fix: Voice-Recording ist durch Discords DAVE-E2EE aktuell kaputt
# (pycord #3139). True = Aufnahme deaktivieren, Bot joint nur (kein Crash).
TEMP_DAVE_FIX = False
LOG_LEVEL = 'INFO'
...
def load_all_settings():
    global AFK_TIMER_MINUTES, TESTING, BOT_LOGS_ID, GARMIN_AUTO_JOIN_CHANNELS, GARMIN_AUTO_JOIN_ENABLED, GARMIN_MAX_RECORDING_DURATION, GARMIN_RECORD_SECONDS, GARMIN_STT_OUTPUT_ENABLED, GARMIN_SILENCE_COMPRESSION_ENABLED, TEMP_DAVE_FIX, HIDDEN_CHANNELS, JOIN_MESSAGE_TIMER_ENABLED, JOIN_MESSAGE_TIMER_MINUTES, JOIN_LOGS_ID, LOG_LEVEL, DISCORD_LOG_LEVEL, PURGE_OLDER_THAN_DAYS, STT_ENABLED, STT_ENGINE, TECHSUPPORT_CHANNEL_ID, TESTING_CHANNEL_ID, VOSK_MODEL_PATH, AFK_CHANNEL_ID, CLEANUP_ALIGNED_RECORDINGS_HOURS, CLEANUP_GARMIN_OUTPUT_HOURS, ORIGINAL_JOIN_LOGS_ID, ORIGINAL_BOT_LOGS_ID, ACCESS_GROUP, JOIN_LOGS_GROUP, SETTINGS_GROUP, BOT_CONTROL_GROUP, RECORDINGS_GROUP
    ...
    JOIN_LOGS_ID = load_channel_id(DB_KEY_JOIN_LOGS_ID, DEFAULT_JOIN_LOGS_ID)
    BOT_LOGS_ID = load_channel_id(DB_KEY_BOT_LOGS_ID, DEFAULT_BOT_LOGS_ID)
    ...
    # --- Web UI access control groups ---
    ACCESS_GROUP = (get_setting(DB_KEY_ACCESS_GROUP, '') or '').strip()
    JOIN_LOGS_GROUP = (get_setting(DB_KEY_JOIN_LOGS_GROUP, '') or '').strip()
    SETTINGS_GROUP = (get_setting(DB_KEY_SETTINGS_GROUP, '') or '').strip()
    BOT_CONTROL_GROUP = (get_setting(DB_KEY_BOT_CONTROL_GROUP, '') or '').strip()
    RECORDINGS_GROUP = (get_setting(DB_KEY_RECORDINGS_GROUP, '') or '').strip()
    ...
    # --- Garmin Silence Compression Settings ---
    garmin_silence_compression_enabled_str = get_setting(DB_KEY_GARMIN_SILENCE_COMPRESSION_ENABLED, os.environ.get('GARMIN_SILENCE_COMPRESSION_ENABLED', 'true'))
    GARMIN_SILENCE_COMPRESSION_ENABLED = garmin_silence_compression_enabled_str.lower() == 'true'

    # --- Temp DAVE Fix (Voice-Recording deaktivieren, pycord #3139) ---
    temp_dave_fix_str = get_setting(DB_KEY_TEMP_DAVE_FIX, os.environ.get('TEMP_DAVE_FIX', 'false'))
    TEMP_DAVE_FIX = temp_dave_fix_str.lower() == 'true'
    ...
    cleanup_aligned_hours_str = get_setting(DB_KEY_CLEANUP_ALIGNED_RECORDINGS_HOURS, '48')
    CLEANUP_ALIGNED_RECORDINGS_HOURS = int(cleanup_aligned_hours_str)

    cleanup_garmin_hours_str = get_setting(DB_KEY_CLEANUP_GARMIN_OUTPUT_HOURS, '72')
    CLEANUP_GARMIN_OUTPUT_HOURS = int(cleanup_garmin_hours_str)
